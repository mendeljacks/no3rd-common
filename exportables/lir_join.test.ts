...
import { path, prop, clone, pathOr, isNil } from 'ramda';
import { assoc_path_mutate } from "../../robo/helpers";
...
describe('lir_join', () => {
    ...
    test('Deeply nests', () => {
        ...
        const { left, inner, right } = lir_join(
            ...
            products,
            images,
            (left_el) => path([ ...left_el, 'id' ], products),
            (l, i, r) => {
        ...
        expect(left.map(left_el => path(left_el, products))).to.deep.equal([ { id: 44, sku: 'tssu1-pink' } ])
        expect(inner).to.deep.equal(goal)
